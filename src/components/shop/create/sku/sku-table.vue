  ...
  <table class="table table-bordered table-sm">
    <thead>
      <tr>
        <th scope="col" class="text-center" style="vertical-align:middle" v-for="(th, thi) in tableThs" :key="thi"
          :rowspan="th.rowspan" :colspan="th.colspan" :width="th.width">{{ th.name }}
        </th>
      </tr>
      <tr>
        <th scope="col" class="text-center" style="vertical-align:middle" v-for="(th, thi) in skuLabels" :key="thi">{{
          th.name }}
        </th>
      </tr>
    </thead>
    <tbody>
      <tr v-for="(item, index) in list" :key="index">
        <td scope="row" v-for="(sku, skuI) in item.skus" :key="skuI" class="text-center">{{ sku.name }}</td>
        ...
        <td class="text-center" width="100px">
          <input type="number" v-model="item.volumn" class="form-control text-center" />
        </td>
        <td class="text-center" width="100px">
